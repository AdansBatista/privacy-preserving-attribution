...
2. **Empirically-anchored three-tier auxiliary-information discipline.** Each bound is reported under a three-tier model: parametric (theorem-level), empirical (Tier 2, illustratively anchored to four-metadata-point joint distributions in the style of @demontjoye2015unique), and worst-case-realistic (Tier 3, including the side-channel inflation of [§6.2](#sec-6-2)). Each section names its tier; the headline reports the regime under which each threat is feasible or infeasible. The empirical anchoring is illustrative rather than operationally calibrated — a deployer using these results must substitute their own auxiliary-information estimates from the relevant joint distributions.
...
- **Tier 3 (worst-case-realistic, headline; illustrative not measured).** Tier 2 plus the side-channel inflation modelled in [§6.2](#sec-6-2) under a Gaussian-jitter timing model (correlated timing plus auxiliary-dataset joining). The headline feasibility / infeasibility statements are reported with the Tier-3 inflation applied; the inflation derivation in [Appendix A.4](#app-a-4) is model-based rather than measured against transaction-rail microdata, and the wide CI on the inflation reflects this. As with Tier 2, a deployer must substitute their own observed timing distribution to operationalise the Tier-3 estimate.
...
**Outputs.** For each period *p* and each jurisdiction *j*, the protocol publishes the noisy aggregate sum

$$ \mathit{AGG}_{p,j} = \left(\sum_{t \in T_{p,j}} \mathit{amount}(t)\right) + \eta_{p,j} $$

where $T_{p,j}$ is the set of in-scope transactions in period *p* with jurisdictional metadata *j*; the sum is computed over the cleartext amounts (via a cryptographic opening of the sum of commitments — see §4.3); and $\eta_{p,j}$ is differential-privacy noise (Gaussian, per [§4.3](#sec-4-3)). The aggregate $\mathit{AGG}_{p,j}$ is the quantity the tax authority uses to allocate jurisdictional revenue.

A typical deployment also publishes a noisy cohort count $\mathit{CNT}_{p,j} = |T_{p,j}| + \eta'_{p,j}$ with its own DP noise term to support attribution audits. **Theorem 2b ([§5.3](#sec-5-3)) bounds amount-disclosure with respect to the sum-side release $\mathit{AGG}_{p,j}$ alone**; the count release is treated as part of the public-information auxiliary channel of Definition 2b, with any leakage about cohort size absorbed into the deployer's $\alpha_a$ specification. A deployment that publishes the count release with its own per-period DP budget $\varepsilon_{\mathrm{count}}$ must independently account for that budget against the deployer's annual privacy goal — the headline ε in this paper refers to the sum-side budget alone. An alternative analysis treating the joint $(\mathit{AGG}_{p,j}, \mathit{CNT}_{p,j})$ release as a vector Gaussian mechanism with sensitivity vector $(\tau, 1)$ and zCDP $\rho_p^{\mathrm{joint}} = \rho_p^{\mathrm{amount}} + \rho_p^{\mathrm{count}}$ is a routine extension of the same Bun-Steinke chain; we keep the sum-only scoping to make the dual-bound separation between amount disclosure and event detection (which the count release does not directly affect) cleaner to read. No other per-transaction information is released.
...
**Definition 2b (Amount-disclosure attack game, sum-side mechanism).** Let $t^*$ be a publicly named target transaction with unknown amount, and let $a_0, a_1 \in [0, \tau]$ be two candidate amounts with $|a_0 - a_1| \leq \tau$ (the bounded-sensitivity bound from B1 below). Let $A$ denote the adversary's auxiliary-information channel about $X_a := \mathit{amount}(t^*)$ in the discriminative-pair $\{a_0, a_1\}$, with TV-distance richness $\alpha_a \in [0, 1]$ as defined in [§3.1](#sec-3-1)(4). The auxiliary channel $A$ may include the count release $\mathit{CNT}_{p,j}$ of [§4.2](#sec-4-2) and any other public-protocol output beyond the sum-side $\mathit{AGG}_{p,j}$. Let $\beta_a \in [0, 1]$ denote the deployer-chosen target advantage threshold for amount disclosure.

The game: a challenger samples $b \in \{0, 1\}$ uniformly, sets $\mathit{amount}(t^*) = a_b$, draws the remaining $k - 1$ amounts from $\mathcal{D}$ on $[0, \tau]$, computes the sum-side release $Y = \sum_{t \in T_{p,j}} \mathrm{clip}_\tau(\mathit{amount}(t)) + \eta$, and presents $(Y, A)$ to $\mathcal{B}$. The adversary outputs $b' \in \{0, 1\}$.
...
**Theorem 2b (Amount disclosure, sum-side release).** Fix $\lambda = 128$. The mechanism analysed by Theorem 2b is the **sum-side release** $\mathit{AGG}_{p,j}$ of [§4.2](#sec-4-2); the count release $\mathit{CNT}_{p,j}$, if present, is treated as part of the public-information auxiliary channel and absorbed into $\alpha_a$ (see [§4.2](#sec-4-2) for the scoping discussion). Under (B1)–(B4) plus:

- *(B5a) — Amount-channel auxiliary information.* The adversary's auxiliary channel about the binary amount discrimination $X_a \in \{a_0, a_1\}$ with $|a_0 - a_1| \leq \tau$ has TV-richness $\alpha_a \in [0, 1]$ as defined in [§3.1](#sec-3-1)(4). Any leakage from the count release $\mathit{CNT}_{p,j}$ — about cohort size, contributing transactions, or scheduling patterns — that is correlated with $X_a$ is absorbed into $\alpha_a$.
- *(B6) — zCDP composition for the sum-side mechanism (single named accountant).* Per-period zCDP parameter for the Gaussian sum-side release: $\rho_p = \varepsilon^2 / (4 \ln(1.25/\delta))$ from @bun2016concentrated Proposition 1.6, where $\varepsilon$ is the sum-side per-period budget. $P$-fold linear composition: $\rho_{\mathrm{total}} = P \cdot \rho_p$ (Lemma 2.3). Conversion to $(\varepsilon_P, \delta_P)$-DP: $\varepsilon_P(\delta_P) = \rho_{\mathrm{total}} + 2\sqrt{\rho_{\mathrm{total}} \log(1/\delta_P)}$ (Proposition 1.3). Total failure $\delta_{\mathrm{total}} = \delta_P + 2^{-100}$. A deployment that publishes the count release with its own per-period DP budget $\varepsilon_{\mathrm{count}}$ must independently account for that budget against the deployer's annual privacy goal; alternatively, a joint vector-Gaussian analysis is a routine extension that adds $\rho_p^{\mathrm{count}}$ to $\rho_p$ above.
...
*Proof.* The sum-side Gaussian mechanism is $\rho_p$-zCDP per (B6) Proposition 1.6, which under linear composition (Lemma 2.3) and conversion (Proposition 1.3) yields the stated $(\varepsilon_P, \delta_P)$-DP guarantee for the $P$-fold sum-side release. The standard hypothesis-testing characterisation of $(\varepsilon, \delta)$-DP gives the single-mechanism advantage $\mathrm{Adv}_{\mathcal{M}}(X_a) \leq (e^{\varepsilon_P} - 1)/(e^{\varepsilon_P} + 1) + \delta_P = \tanh(\varepsilon_P/2) + \delta_P$ on the discriminative pair $(a_0, a_1)$ [@kairouz2017composition]. Lemma 1 with the auxiliary channel (B5a) — which absorbs any leakage from the count release — gives the stated additive composition with $\alpha_a$. The $\min(\cdot, 1)$ is the trivial bound on binary advantage. The bound depends on $\tau$ only through (B1)'s sensitivity and is invariant to cohort size — adding records to the database does not dilute a per-neighbouring-database DP guarantee. ∎
...
*Honest gap statement.* Against an adversary exploiting the heavy tail of the *unclipped* distribution, Theorem 2a's bound on the *clipped* aggregate is the operative statement; clipping leakage is absorbed into $\delta_{\mathrm{total}}$ as above. The i.i.d.\ assumption (B3) is an idealization — real cohorts have correlated transaction patterns that may amplify the event-detection signal beyond what an i.i.d.\ model predicts. [§6.2](#sec-6-2) analyses one such amplification (correlated timing plus auxiliary-dataset joining); the gap to fully-general adversarial cohort distributions is named explicitly in [§8.5](#sec-8-5).
...
|---:|---:|---:|---:|
| 0.25 | \$211,952 | 0.87 | DP-dominated |
...
| Tier 2 $\alpha_e \approx 0.94$ from de Montjoye-style spatiotemporal uniqueness | **Illustrative anchor** | [§3.1](#sec-3-1)(4); [@demontjoye2015unique] |
...
| Side-channel inflation under correlated timing + join | **Simulation-backed (illustrative)** | [§6.2](#sec-6-2); [Appendix A.4](#app-a-4) |
...
## 6.2 Worst-case-realistic side-channel {#sec-6-2}
...
4. The auxiliary-information richness $\alpha_e$ for event detection is credibly within Tier 1, which under [§6.2](#sec-6-2)'s side-channel analysis requires either weak adjacent-layer auxiliary information *or* effective adjacent-layer mitigations (onion routing, padding, pseudonym rotation) that drive empirically-realistic $\alpha_e$ down to Tier 1.
...
Theorem 2a and 2b are protocol-layer bounds. Real-world privacy is bounded *above* by adjacent-layer leakage (timing patterns, network correlation, pseudonym reuse) and *below* by the protocol-layer bound. The deployer's actual privacy posture is the composition. [§6.2](#sec-6-2)'s worst-case-realistic side-channel analysis already demonstrates that the empirically-realistic regime drives Theorem 2a's $\alpha_e$ into Corollary 2a's infeasibility region at typical $\beta_e$ — the positive event-detection result of Theorem 2a is meaningful only when adjacent-layer mitigations are in place.
...
This appendix contains the full proofs of Theorems 1, 2, and 3, the derivation of the corollary *k\** bound under the new framework, and the side-channel inflation factor. All numerical values reported in the paper come from the R script `derivation/2-theorem2-zcdp-code.R`, which is the audit-trail companion to this appendix.
...
### Step 2b — Theorem 2b: amount-disclosure bound (zCDP composition, sum-side mechanism)

The mechanism analysed by Theorem 2b is the per-period sum-side Gaussian release $\mathit{AGG}_{p,j}$ of [§4.2](#sec-4-2) with sensitivity $\Delta = \tau$ and noise scale $\sigma_{\mathrm{DP}}$. Any companion releases — most notably the count release $\mathit{CNT}_{p,j}$, if published — are *not* part of the mechanism this proof bounds; they are absorbed into the auxiliary channel (B5a) and therefore enter the bound through $\alpha_a$ rather than through $\rho_p$.

The sum-side Gaussian mechanism satisfies $\rho_p$-zCDP with $\rho_p = \Delta^2 / (2\sigma_{\mathrm{DP}}^2) = \varepsilon^2 / (4 \ln(1.25/\delta))$ (@bun2016concentrated Proposition 1.6, tight for the Gaussian mechanism). $P$-fold linear composition gives $\rho_{\mathrm{total}} = P \cdot \rho_p$ (Lemma 2.3). Conversion to $(\varepsilon_P, \delta_P)$-DP gives $\varepsilon_P(\delta_P) = \rho_{\mathrm{total}} + 2\sqrt{\rho_{\mathrm{total}} \log(1/\delta_P)}$ (Proposition 1.3). For headline parameters ($\varepsilon = 1.0$, $\delta = 10^{-6}$, $P = 12$, $\delta_P = 10^{-6}$), $\rho_{\mathrm{total}} = 0.2137$ and $\varepsilon_P = 3.6502$. (RDP cross-check at the optimal Rényi order [@mironov2017renyi]: 3.6502, agreement to relative difference $1.2 \times 10^{-5}$.)

**Note on the joint vector-Gaussian extension.** A deployer who wishes to bound amount-disclosure under the joint $(\mathit{AGG}_{p,j}, \mathit{CNT}_{p,j})$ release directly (rather than absorbing the count release into $\alpha_a$) extends the same Bun-Steinke chain by computing the joint per-period parameter $\rho_p^{\mathrm{joint}} = \Delta^2/(2\sigma_{\mathrm{DP}}^2) + 1/(2\sigma_{\mathrm{count}}^2)$ where $\sigma_{\mathrm{count}}$ is the count-side noise scale. The downstream conversion and TV-chain-rule application are unchanged in form; the headline $\varepsilon_P$ inflates by an amount depending on the budget split between the sum and count releases. We keep the sum-side scoping in the main statement to make the dual-bound separation cleaner; the joint-mechanism extension is straightforward.

Under the amount-disclosure attack game (Definition 2b), the discriminative pair is $(a_0, a_1)$ with $|a_0 - a_1| \leq \tau$. The standard hypothesis-testing characterisation of $(\varepsilon_P, \delta_P)$-DP [@kairouz2017composition] gives the optimal binary-distinguisher TV distance for the sum-side mechanism

$$ \mathrm{Adv}_{\mathcal{M}}(X_a) \;\leq\; \frac{e^{\varepsilon_P} - 1}{e^{\varepsilon_P} + 1} + \delta_P \;=\; \tanh(\varepsilon_P/2) + \delta_P. $$

The bound is $k$-independent: it is the per-neighbouring-database guarantee, and adding records to the database does not dilute it. Lemma 1 with auxiliary channel (B5a) — which absorbs the count release and any other public-protocol auxiliary information — gives
...
**Headline numerical estimate.** For $\varepsilon = 1.0$, $\delta = 10^{-6}$, $P = 12$, $\delta_P = 10^{-6}$, $\tau = \$10{,}000$, $\sigma_T = \$1{,}000$, $\alpha_e = 0.05$, $\beta_e = 0.10$: $\sigma_{\mathrm{DP}} \approx \$52{,}988$; $z^* = \Phi^{-1}(0.525) \approx 0.0627$; $(\tau/(2z^*))^2 - \sigma_{\mathrm{DP}}^2 \approx 3.55 \times 10^9$; $k^*_{\mathrm{event}} \approx 3{,}551$ (intermediate decimals shown to two significant figures; full-precision computation in the audit-traceable R script yields 3,551 exactly). Regime crossover at $k = \sigma_{\mathrm{DP}}^2 / \sigma_T^2 \approx 2{,}808$.
...
Under the [§6.2](#sec-6-2) combined attack, the adversary's effective $\alpha_e$ is inflated by the timing-channel mutual information about target identity. We derive the inflation step-by-step under stated modelling assumptions; the result is illustrative rather than measured.
...
The audit-traceable R derivation script (closed-form bounds, sensitivity tables, Monte Carlo verification) and the Python attack-simulation workbench are distributed under `derivation/` and `simulations/` in this repository (`https://github.com/AdansBatista/privacy-preserving-attribution`). The git tag `eprint-v1` on that repository corresponds to the IACR ePrint version of record.
